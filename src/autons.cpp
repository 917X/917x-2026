...
void skills() {
	...
	// chassis.pid_wait_quick_chain();
	// chassis.pid_drive_set(29.5_in,40,true);
	...
	// liftPiston.set(true);
	// chassis.pid_wait_quick_chain();
	// chassis.pid_drive_set(-8.8_in,70,true);
	// chassis.pid_wait_quick();
	// intake.set(Intake::IntakeState::SCORE,65,127);
	// pros::delay(500);
	// intake.set(Intake::IntakeState::OUTTAKE,127);
	// pros::delay(100);
	// intake.set(Intake::IntakeState::SCORE,65,127);
	...
	// intake.set(Intake::IntakeState::INTAKE,127);
	// chassis.pid_odom_ptp_set({{46_in,-46.1_in},fwd,80},true); //46,46.4
	// liftPiston.set(false);
	// chassis.pid_wait_quick_chain();

	// chassis.pid_turn_set(88,127);
	// chassis.pid_wait();
	// chassis.pid_drive_set(14.7_in,80,true);
	// chassis.pid_wait();
	// pros::delay(500);
	// //localizing
	// x = (71 - 14.3125)*1_in;
	...
	//SCORING BALLS FROM PARK
	chassis.pid_odom_ptp_set({{40_in,46.8_in},fwd,90},true);
	chassis.pid_wait();
	chassis.pid_turn_set(90,127);
	chassis.pid_wait();
	...
	chassis.pid_wait();
	chassis.pid_odom_ptp_set({{-18_in,59_in},fwd,90},true);
	chassis.pid_wait();
	chassis.pid_odom_ptp_set({{-42_in,44.8_in},fwd,75},true);
	chassis.pid_wait();

	chassis.pid_turn_set(-90,127);
	chassis.pid_wait();
	//SCORING
	chassis.pid_drive_set(-25.5_in,100,true);
	...
	chassis.pid_wait();
	intake.set(Intake::INTAKE,127);
	chassis.pid_drive_set(32.3_in,60,true,false);
	loaderPiston.set(true);
	chassis.pid_wait();
	x = (-71 + 14.3125)*1_in;
	...
	chassis.pid_wait_quick_chain();
	intake.set(Intake::SCORE,127,85); 
	pros::delay(1000);
	loaderPiston.set(false);

	// //GO TO FINAL MATCHLOADER
	// chassis.pid_odom_ptp_set({{-46_in,-48_in},fwd,180},true);
	// chassis.pid_wait();
	// chassis.pid_turn_set(-90,120);
	// chassis.pid_wait();
	// loaderPiston.set(true);
	// chassis.pid_drive_set(13_in,80,false);
	// chassis.pid_wait();
	// pros::delay(1000);
