...
void left_elims() {
	...
	intake.set(Intake::IntakeState::INTAKE, 127);
	chassis.pid_odom_ptp_set({{39_in, -41.5_in}, fwd, 110}, true);
	liftPiston.set(false);
    chassis.pid_wait_quick_chain();
	...
	chassis.pid_swing_set(ez::LEFT_SWING, 158_deg, 110, 3, false);
    chassis.pid_wait_quick_chain();
...
void right_elims() {
	...
	chassis.pid_odom_ptp_set({{-46_in,-46.5_in},fwd,127},false); //-46,48
	chassis.pid_wait_quick();
	chassis.pid_turn_set(-90,127);
	chassis.pid_wait_quick();
...
void skills() {
	...
	intake.set(Intake::IntakeState::INTAKE,127);
	chassis.pid_odom_ptp_set({{46_in,-46_in},fwd,80},true); //46,46.4
	liftPiston.set(false);
	chassis.pid_wait_quick_chain();
	...
	chassis.pid_odom_ptp_set({{40_in,44_in},fwd,90},true);
	chassis.pid_wait();
	chassis.pid_turn_set(90,127);
	chassis.pid_wait();
	loaderPiston.set(true);
	...
	// chassis.pid_wait_quick_chain();
	// pros::delay(500);
	chassis.pid_drive_set(23_in,60,true,false); //18.5
	chassis.pid_wait();
	intake.set(Intake::INTAKE,127);
	...
	chassis.pid_wait();
	chassis.pid_odom_ptp_set({{-42_in,47.4_in},fwd,75},true);
	chassis.pid_wait();
	...
	chassis.pid_wait_quick_chain();
	intake.set(Intake::SCORE,100,127); 
	pros::delay(1200);
	loaderPiston.set(false);
	intake.set(Intake::INTAKE,127);
	...
	chassis.pid_wait_quick_chain();
	chassis.pid_odom_ptp_set({{-41_in,-48_in},fwd,70},true);
	chassis.pid_wait();
	chassis.pid_turn_set(-90,127);
	chassis.pid_wait();
	...
	chassis.pid_wait();
	chassis.pid_swing_set(ez::LEFT_SWING,0,127,-5);
	chassis.pid_wait();
	chassis.pid_drive_set(35_in,100,false,false);
	chassis.pid_wait();
