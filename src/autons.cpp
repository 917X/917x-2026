...
void solo_awp(){
  ...
  leverProfiler.setProfile({{-10, 127},{60,80},{100,67}});
	...
	chassis.pid_odom_ptp_set({{-42_in,-48_in},rev,80},false); //-46,48
	chassis.pid_wait_quick();
	chassis.pid_turn_set(-90,100);
	chassis.pid_wait_quick();
	loaderPiston.set(true);
	pros::delay(200);
	chassis.pid_drive_set(15.7_in,52,false, true, -89);
	chassis.pid_wait_quick();

    // score first batch
	// chassis.pid_drive_set(-27_in,127,false, true, -90);
	chassis.pid_odom_ptp_set({{-27_in,-50.067_in},rev,80},false);
    ...
    loaderPiston.set(false);
    chassis.pid_odom_ptp_set({{-24_in,13_in},fwd,70},false); //-24,13.5
	chassis.pid_wait_until({-23_in,3_in});
    chassis.pid_wait_quick();
    
    chassis.pid_odom_ptp_set({{-40_in,42.3_in},fwd,85},true); //-40,45.2
    chassis.pid_wait_quick();
  ...
  leverProfiler.setProfile({{-10, 60}, {50, 80}});
  ...
  loaderPiston.set(true);
	...
	//Score middle goal
	chassis.pid_drive_set(-8,120,true);
...
void left_elims() {
	...
	chassis.pid_odom_ptp_set({{-24,24},fwd,80},true,12,50);
	chassis.pid_wait_until(10);
	loaderPiston.set(true);
	chassis.pid_wait_quick_chain(3);
	chassis.pid_turn_set({-46,53},rev,90);
	chassis.pid_wait_quick();
	chassis.pid_drive_set(-8_in,100,80,false,false);
	chassis.pid_wait_quick_chain(4);

	chassis.pid_swing_exit_condition_set(25_ms, 2_deg, 25_ms, 7_deg, 50_ms,
										 50_ms);
	chassis.pid_swing_set(ez::RIGHT_SWING,-90_deg,100,10,true);
	chassis.pid_wait_quick();
...
void right_4_rush(){
	...
	chassis.pid_odom_ptp_set({{-29,-24},fwd,80},true,12,50);
	chassis.pid_wait_until(10);
	loaderPiston.set(true);
	chassis.pid_wait_quick_chain(3);
	chassis.pid_turn_set({-46,-53},rev,90);
...
void left_4_rush(){
  chassis.pid_drive_exit_condition_set(90_ms, 1_in, 250_ms, 3_in, 60_ms,
    ...
    flapperPiston.set(true);
    intake.set(Intake::IntakeState::INTAKE, 127);
    chassis.odom_xyt_set(-56.5_in, 17_in, 90_deg);  // Y flipped, heading flipped

    leverProfiler.setProfile({{-10,120},{60,90}});

    chassis.pid_odom_ptp_set({{-28, 22},fwd,100},true);  // Y flipped
    chassis.pid_wait_until(10);
    loaderPiston.set(true);
    chassis.pid_wait_quick_chain(3);
    chassis.pid_turn_set({-50, 52},rev,90);  // Y flipped
    chassis.pid_wait_quick();
    chassis.pid_drive_set(-12_in,100,80,false,false);
    chassis.pid_wait_quick_chain(4);

    chassis.pid_swing_exit_condition_set(25_ms, 2_deg, 25_ms, 7_deg, 25_ms,
                                         25_ms);
    chassis.pid_swing_set(ez::RIGHT_SWING, 90_deg, 100, 10, true);  // LEFT→RIGHT, angle flipped
    chassis.pid_wait_quick();
    chassis.pid_swing_exit_condition_set(90_ms, 2_deg, 250_ms, 7_deg, 500_ms,
                                         500_ms);
    loaderPiston.set(false);
    ...
    intake.set(Intake::IntakeState::SCORE, 127);
    pros::delay(200);
    chassis.pid_swing_set(ez::RIGHT_SWING, 24, 100, -20, true);  // LEFT→RIGHT, angle flipped
    chassis.pid_wait_quick();
    chassis.pid_swing_set(ez::LEFT_SWING, 90, 100, 0, true);  // RIGHT→LEFT, angle flipped
    chassis.pid_wait_quick();
    ...
    chassis.pid_swing_set(ez::LEFT_SWING, -140, 127, -5, true);  // RIGHT→LEFT, angle flipped
...
void right_4_loader_rush(){
  ...
  flapperPiston.set(false);         
	chassis.pid_drive_set(-27_in, 100, 80, false, false);
...
void right_7_rush(){
	...
	chassis.pid_odom_ptp_set({{-40,-39.3},fwd,90},true); //-9.5,35
	// loaderPiston.set(false);
	...
	pros::delay(200);
	chassis.pid_wait_quick_chain();
	chassis.pid_drive_set(12_in,60,false, true, -90);
	chassis.pid_wait_quick();
	pros::delay(50);
...
void right_elims() {
  leverProfiler.setProfile({{-10, 127},{60,90},{100,67}});
	...
	chassis.pid_odom_ptp_set({{-43_in,-45.5_in},fwd,90},true); //-46,48
	chassis.pid_wait_quick();
	...
	chassis.pid_drive_set(13.7_in,60,false, true, -90); 
	chassis.pid_wait_quick();
    ...
    intake.set(Intake::IntakeState::AUTON_SCORING);
    pros::delay(690);
    intake.set(Intake::IntakeState::RAW, -50);
	...
	chassis.pid_wait_quick();
	intake.set(Intake::IntakeState::INTAKE);
	chassis.pid_odom_ptp_set({{-17,-21},fwd,55},false);
	chassis.pid_wait_quick_chain();
    chassis.pid_odom_ptp_set({{-14,-17},fwd,53},false);
	chassis.pid_wait_quick();
	intake.set(Intake::IntakeState::OUTTAKE,60);
	pros::delay(900);
...
void skills_83_route() {
    ...
    chassis.pid_odom_drive_exit_condition_set(100_ms, 1_in, 150_ms, 5_in, 150_ms, 500_ms);
	okapi::QLength x;
	okapi::QLength y;
	flapperPiston.set(true);
	...
	chassis.pid_odom_ptp_set({{-27,20}, fwd,60}, true, 17, 25); //28,18
	...
	chassis.pid_wait_quick();
	chassis.pid_odom_ptp_set({{-36,42},fwd,80}, true);  //-36,43.5
	chassis.pid_wait_quick();
	chassis.pid_turn_set(-90,100);
	chassis.pid_wait_quick();
	...
	chassis.pid_wait_quick();
	chassis.pid_drive_set(28.5_in,52, false, true, -90);
	chassis.pid_wait_until(10);
	// liftPiston.set(true);
	hoodPiston.set(false);
	intake.set(Intake::IntakeState::INTAKE,127);
	...
	chassis.pid_odom_ptp_set({{40,45},rev,75}, true);
    chassis.pid_wait_quick();
	...
	chassis.pid_drive_set(29.2_in,52, false, true, 92);
	...
	chassis.pid_turn_set(90,127);
	chassis.pid_wait_quick();
	chassis.pid_odom_ptp_set({{24_in,-49_in},rev,80},true); //46,46
	chassis.pid_wait_quick();
	pros::delay(500);
	intake.set(Intake::IntakeState::AUTON_SCORING,127);
	chassis.drive_set(-20,-20);
	...
	chassis.pid_wait_quick();
	hoodPiston.set(false);
	chassis.pid_drive_set(29.5_in,57, false, true, 90);
	// liftPiston.set(true);
    intake.set(Intake::IntakeState::INTAKE,127);
	chassis.pid_wait_quick();
	pros::delay(1000);
	...
	chassis.pid_drive_set(29.2_in,55, false, true, -89);
	...
	chassis.pid_wait_quick_chain(4);
	chassis.pid_odom_ptp_set({{-63,-20.5},fwd,60}, true); //61.5,21
	chassis.pid_wait_quick_chain(3);
	loaderPiston.set(true);
