...
Intake::Intake(pros::Motor &indexerMotor, pros::Optical &colorSort,
			   pros::Motor &topRoller, pros::Motor &middleRoller,
			   pros::Motor &bottomRoller)
	: indexerMotor(indexerMotor), colorSort(colorSort), frontRoller(topRoller),
	  middleRoller(middleRoller), bottomRoller(bottomRoller) {
...
void Intake::setSeparation(Ball ball) { this->ball = ball; }
...
void Intake::intakeControl() {
	while (true) {
		...
		//     checkForSort();
		// }
		// bool COMPLETED_MOVEMENT = (INITIAL_POSITION + SEPARATION_MOVEMENT) <
		// middleRoller.get_position();
		...
		case INTAKING:
			...
			middleRoller.move(topFull? 20 : speed);
